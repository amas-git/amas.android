...
public class vnode {
    ...
    public static vnode createFromString(rules r, String content) {
        ...
        String[] xs = content.split("\\s+");
        vnode node = new vnode();
        node.r = r;

        for(int i=0; i<xs.length; ++i) {
            rules_item rule = r.get(i);
            if(rule.skip()) {
                continue;
            }

            if (rule.type == Integer.class) {
                node.addInteger(rule.name, Integer.valueOf(xs[i]));
            } else if (rule.type == Long.class) {
                node.addLong(rule.name, Long.valueOf(xs[i]));
            } else {
                node.addString(rule.name, xs[i]);
            }
        }
        return node;
    }
    ...
    public static class rules_item {
        public rules_item(String name, vnode.op op, Class<?> type) {
            this.name = name;
            this.op = op;
            this.type = type;
        }
