...
public class proc_pid_stat extends proc {
	static vnode.rules rule_stat = new vnode.rules();
	{
		...
		rule_stat.notchanged("22 STARTTIME");
	...
	public proc_pid_stat update() {
		...
		String content = OS.exec(String.format(OS.SHELL+"cat /proc/%s/stat", pid));
		//System.err.println(""+pid+" -> " + content);
		node = vnode.createFromString(rule_stat, content);
		return this;
	}
