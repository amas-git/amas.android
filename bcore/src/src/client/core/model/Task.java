...
public class Task implements Callable<Event>, Comparable<Task> {
	...
	public void addTo(String lgUri) {
		if (mEventTo == null) {
			mEventTo = new Notifiers();
		}
...
/*	public void setWeight(int weight) {
	...
	final public Event call() throws java.io.IOException {
		...
		Event event = process(); 
		//System.out.println("Core       "+"Task Result >>> " + event);
		touch(event);
	...
	public String toString() {
		return String.format("(%s (:desc %s) (:weight %d) (:sec %f))", getClass().getSimpleName()+"@"+hashCode()+"/"+mId , mDesc, mWeight, mTimeStamp.getLifeTimeSec());
	}
	
	public void setTo(Notifiers eventTo) {
		mEventTo = eventTo;
	}
