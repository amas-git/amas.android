...
public class EventManager {
	...
	protected class EventDispather extends Dispatcher<Event> {
		public EventDispather(BlockingQueue<Event> queue) {
			mQueue   = queue;
			mWorkers = Executors.newFixedThreadPool(Core.EM_DIPATCHER_WORKERS);
		...
		public Event onDispatch(Event event) {	
			...
			Core.LOG.d("DISPACHER >>>", event.toString());
	...
	public boolean existed(String uri) {
		return  mListenerGroups.get(uri) != null;
	}
