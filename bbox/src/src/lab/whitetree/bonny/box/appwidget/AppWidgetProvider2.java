...
public class AppWidgetProvider2 extends BaseWidgetProvider {
	protected static ComponentName SELF = new ComponentName("lab.whitetree.bonny.box", "lab.whitetree.bonny.box.appwidget.AppWidgetProvider2");
	public static final int ON_CLICK = 1;
	public static final int W = MoomParser.DP_TO_PIX(164); // XXX: 此值必须与
	public static final int H = MoomParser.DP_TO_PIX(164);

	private static ArrayList<Integer> sAppWidget2Ids = null;

	@Override
	public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
		synchronized (this) {
			sAppWidget2Ids = addToWidgetIdList(sAppWidget2Ids, appWidgetIds);
		}
	...
	private static ArrayList<Integer> sTemp2Ids = null;
	public static synchronized void updateWidget(Context context) {
		try {
			if (sAppWidget2Ids == null) {
				return;
			}
			sTemp2Ids = (ArrayList<Integer>) sAppWidget2Ids.clone();

			final AppWidgetManager gm = AppWidgetManager.getInstance(context);
			for (int widgetId : sTemp2Ids) {
				RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.appwidget_2);
				...
				if (!TextUtils.isEmpty(url)) {
					...
					views.setImageViewResource(R.id.moom_widget,
							R.drawable.not_avail);
				}
