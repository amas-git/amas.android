...
public class MySectionIndexer implements android.widget.SectionIndexer{
    ...
    public MySectionIndexer(String[] sections, int[] counts) {
        ...
        for (int i = 0; i < counts.length; i++) {
            ...
            android.util.Log.i("MySectionIndexer", "counts["+i+"]:"+counts[i]);
        }
        allCount = position;
        mCounts = counts;
    }
    ...
    public int getSectionForPosition(int position) {
        if (position < 0 || position >= allCount) {
            return -1;
        }
        //注意这个方法的返回值，它就是index<0时，返回-index-2的原因
        //解释Arrays.binarySearch，如果搜索结果在数组中，刚返回它在数组中的索引，如果不在，刚返回第一个比它大的索引的负数-1
        //如果没弄明白，请自己想查看api
        int index = java.util.Arrays.binarySearch(mPositions, position);
        index = index >= 0 ? index : -index - 2; //当index小于0时，返回-index-2，
        if(mCounts[index]==0){
            for(int i=index;i<mCounts.length;i++) {
                if (mCounts[i]!=0) {
                    index = i;
                    break;
                }
