 ...
 * Created by Rocky on 2015/7/22  10:38
 * PACKAGE_NAME com.keniu.security.main.widget
...
public class CommonSwitchButton extends CompoundButton {
    ...
    private OnCheckedChangeListener mOnCheckedChangeListener = new OnCheckedChangeListener() {
        ...
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
            if(mOnClickListener != null){
                mOnClickListener.onClick(CommonSwitchButton.this);
            }
        }
    };

    private boolean mCanDispathTouch = false;
    ...
    public CommonSwitchButton(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs,R.styleable.onews__common_switchbutton);

        mConf.setThumbMarginInPixel(ta.getDimensionPixelSize(
                R.styleable.onews__common_switchbutton_thumb_margin,
                mConf.getDefaultThumbMarginInPixel()));
        mConf.setThumbMarginInPixel(ta.getDimensionPixelSize(
                R.styleable.onews__common_switchbutton_thumb_marginTop,
                mConf.getThumbMarginTop()), ta.getDimensionPixelSize(
                R.styleable.onews__common_switchbutton_thumb_marginBottom,
                mConf.getThumbMarginBottom()), ta.getDimensionPixelSize(
                R.styleable.onews__common_switchbutton_thumb_marginLeft,
                mConf.getThumbMarginLeft()), ta.getDimensionPixelSize(
                R.styleable.onews__common_switchbutton_thumb_marginRight,
                mConf.getThumbMarginRight()));
        mConf.setRadius(ta.getInt(
                R.styleable.onews__common_switchbutton_switch_radius,
                SwitchButtonConfiguration.Default.DEFAULT_RADIUS));

        mConf.setChecked((ta.getBoolean(
                R.styleable.onews__common_switchbutton_switch_checked,
                SwitchButtonConfiguration.Default.DEFAULT_CHECKED)));

        mConf.setThumbWidthAndHeightInPixel(ta.getDimensionPixelSize(
                R.styleable.onews__common_switchbutton_thumb_width, -1), ta.getDimensionPixelSize(
            R.styleable.onews__common_switchbutton_thumb_height, -1));

        mConf.setMeasureFactor(ta.getFloat(R.styleable.onews__common_switchbutton_measureFactor, -1));

        mConf.setInsetBounds(
                ta.getDimensionPixelSize(R.styleable.onews__common_switchbutton_insetLeft, 0),
                ta.getDimensionPixelSize(R.styleable.onews__common_switchbutton_insetTop, 0),
                ta.getDimensionPixelSize(R.styleable.onews__common_switchbutton_insetRight, 0),
                ta.getDimensionPixelSize(R.styleable.onews__common_switchbutton_insetBottom,0));

        int velocity = ta.getInteger(R.styleable.onews__common_switchbutton_animationVelocity, -1);
        mAnimationController.setVelocity(velocity);

        //新加的方法
        setChecked(mConf.getChecked());
    ...
    private void initView() {
        mConf = SwitchButtonConfiguration.getDefault(getContext()
    ...
    private void fetchDrawableFromAttr(TypedArray ta) {
        ...
        mConf.setOffDrawable(fetchDrawable(ta,
                R.styleable.onews__common_switchbutton_offDrawable,
                R.styleable.onews__common_switchbutton_offColor,
                SwitchButtonConfiguration.Default.DEFAULT_OFF_COLOR));
        mConf.setOnDrawable(fetchDrawable(ta,
                R.styleable.onews__common_switchbutton_onDrawable,
                R.styleable.onews__common_switchbutton_onColor,
                SwitchButtonConfiguration.Default.DEFAULT_ON_COLOR));
    ...
    private Drawable fetchThumbDrawable(TypedArray ta) {
        ...
        Drawable tempDrawable = ta.getDrawable(R.styleable.onews__common_switchbutton_thumbDrawable);
        if (tempDrawable != null) {
            return tempDrawable;
        }

        int normalColor = ta.getColor(
                R.styleable.onews__common_switchbutton_thumbColor,
                SwitchButtonConfiguration.Default.DEFAULT_THUMB_COLOR);
        int pressedColor = ta.getColor(
                R.styleable.onews__common_switchbutton_thumbPressedColor,
                SwitchButtonConfiguration.Default.DEFAULT_THUMB_PRESSED_COLOR);
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (deltaX < mTouchSlop && deltaY < mTouchSlop
                    ...
                    slideToChecked(nextStatus, true);
                }
    ...
    public void toggle(boolean animated) {
        if (animated) {
            slideToChecked(!mIsChecked, false);
        } else {
    ...
    private void setCheckedInClass(boolean checked, boolean trigger) {
        ...
        if (mOnCheckedChangeListener != null && trigger && mExeChangedListener) {
    ...
    public void slideToChecked(boolean checked) {
        slideToChecked(checked, false);
    }

    private boolean mExeChangedListener = false;
    private void slideToChecked(boolean checked, boolean exechangeListener) {
        if (isAnimating) {
            return;
        }
        mExeChangedListener = exechangeListener;
        //新加的判断
        if (mThumbZone == null) {
    ...
    class SBAnimationListener implements SwitchAnimationController.OnAnimateListener {
